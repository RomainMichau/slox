...
object Interpreter {
  ...
  private def executeBlock(block: Block, parentEnv: Environment): ValidatedNel[InterpreterRuntimeError, Any] = {
    ...
    stmts.headOption match {
      case Some(head) =>
        stmts.tail.foldLeft(execute(head, newEnv)) { case (acc, stmt) => acc.andThen(_ => execute(stmt, newEnv)) }
      case None => ().validNel
    }
  }
  ...
  private def evaluate(expr: Expr, env: Environment): ValidatedNel[InterpreterRuntimeError, Any] = {
    expr match {
      ...
      case Binary(leftEx, operator, rightEx) =>
        ...
        operator match {
          case op: MINUS =>
            (leftAna, rightAna).tupled.andThen {
              case (l: Double, r: Double) => (l - r).validNel
              case (left, right) =>
                InterpreterRuntimeError(
                  op,
                  s"Operands must be numbers for subtraction. Got ${left.getClass.getSimpleName} and ${right.getClass.getSimpleName}."
                ).invalidNel
            }
          case op: STAR =>
            (leftAna, rightAna).tupled.andThen {
              ...
              case (left, right) =>
                InterpreterRuntimeError(
                  op,
                  s"Operands must be numbers for multiplication. Got ${left.getClass.getSimpleName} and ${right.getClass.getSimpleName}."
                ).invalidNel
            }
          case op: SLASH =>
            (leftAna, rightAna).tupled.andThen {
              ...
              case (left, right) =>
                InterpreterRuntimeError(
                  op,
                  s"Operands must be numbers for division. Got ${left.getClass.getSimpleName} and ${right.getClass.getSimpleName}."
                ).invalidNel
            }
          case op: PLUS =>
            (leftAna, rightAna).tupled.andThen {
              ...
              case (left, right) =>
                InterpreterRuntimeError(
                  op,
                  s"Operands must be two numbers or two strings for addition. Got ${left.getClass.getSimpleName} and ${right.getClass.getSimpleName}."
                ).invalidNel
            }
          case op: GREATER =>
            (leftAna, rightAna).tupled.andThen {
              ...
              case (left, right) =>
                InterpreterRuntimeError(
                  op,
                  s"Operands must be numbers for comparison. Got ${left.getClass.getSimpleName} and ${right.getClass.getSimpleName}."
                ).invalidNel
            }
          ...
          case _: BANG_EQUAL  => (leftAna, rightAna).mapN((left, right) => left != right)
          case _: EQUAL_EQUAL => (leftAna, rightAna).mapN((left, right) => left == right)
        }
      case Variable(name) =>
        env.get(name) match {
      ...
      case Assign(name, valueExpr) =>
        evaluate(valueExpr, env).andThen { value =>
